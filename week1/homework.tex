...
There exists a natural morphism $x_{YZ}: X_1 \times_Y X_2 \rightarrow X_1 \times_Z X_2$ as proven in the previous exercise. Call this morphism $x_{YZ}$.
...
Consider the set $YY =:\{(y,y)|y\in Y\}$. We have an obvious bijection between $YY$ and $Y$, and by the universal property of $Y \times_Z Y$ a unique map $j: YY \rightarrow Y \times_Z Y$. Thus a map $\beta = j \circ y \mapsto (y,y): Y \times_Z Y \rightarrow Y$ exists.asd

If we define the map from $X_1 \times_Z X_2 \rightarrow Y \times_Z Y$ as $y\mapsto y,y \circ a \circ pr_X1$ then the diagram commutes and the maps $ \circ x_{XY}$
